...
it('should get all users', async (t) => {
    //setup
    const usersToInsert = [
        { fullName: 'omar ibrahim', age: 25, job: 'engineer' },
        { fullName: 'mohamed shokry', age: 24, job: 'developer' },
    ];
...
it('should get a single user by id', async (t) => {
    // Setup
    const insertedUser = await User.create({ fullName: 'omar ibrahim', age: 25 });
...
it('should return USER NOT FOUND for non-existent user', async (t) => {
    // Setup
    const nonExistentId = '345'; // Invalid ID
...
it('deleteUser should delete a user by id', async (t) => {
    ...
    const user = await User.create({ fullName: 'hassan mohamed', age: 30 });
  
    // Call deleteUser controller
    const deletedUser = await deleteUser({ params: { id: user._id.toString() } });
  
    // Verify
    t.is(deletedUser.fullName, 'hassan mohamed');
